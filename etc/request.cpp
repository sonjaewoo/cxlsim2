...
Request::Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, tlm::tlm_generic_payload *trans):
addr(addr), type_id(type), source_id(source_id), callback(callback), trans(trans) {};
}        // namespace Ramulator
